...
function NomineeDetails() {
    ...
    return (
        <div className="px-6 py-2 w-full overflow-scroll min-h-min md:h-full bg-gray-50" >


            <div className='flex items-center gap-2'>
                <FaTree className='text-secondary' size={25} />
                <span className='text-lg font-semibold  gap-2 text-black'>
                    {isView ? "Add Nominee" : "Nominee Details"}
                </span>
            </div>
            <div className="mt-5">
                {
                    isView && <>
                        <p className='mb-1 font-normal text-black'>{isView ? "Add" : "Update"} Your Nominee Details</p>
                ...
                {/* <CustomButton onClick={() => setIsView(!isView)} title={` ${isView ? "Add" : "Update"} Your Nominee`} startIcon={<FaUsersRectangle />} className={"!mt-2"} /> */}
